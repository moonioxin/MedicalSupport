...
(function($) {
	...
	/**
	 * check the date if in the holidayArray fix by pong
	 */
	function checkHoliday (date){
		if(holidayArray.toString().indexOf(date) > -1) {
			return true;
		}else{
			return false;
		}
	...
	/**
	 * trans the date like 2013-01-01 FIX BY PONG
	 */
	function transDate(date){
		var calToday;
		var calCurrentYear  = date.getFullYear();
		var calCurrentMonth = date.getMonth();
		var calCurrentDay   = date.getDate();
		...
		*/
		...
		this.do_init = function(){
			...
			if (pos != -1) { // 如果pos值为-1则说明搜索"version="失败
				...
				language = unescape(value); // 对它解码
			} else {
				language = ""; // 搜索失败，返回空字符串
			
		    }

			if (language=="CN"){
				Calendar.dayNames = new Array("周日","周一","周二","周三","周四","周五","周六");
			} else if (language=="JP"){
				Calendar.dayNames = new Array("日曜日","月曜日","火曜日","水曜日","木曜日","金曜日","土曜日");
			} else {
				Calendar.dayNames = new Array("周日","周一","周二","周三","周四","周五","周六");
			}
			...
			for(var dayIndex = 0; dayIndex < Calendar.dayNames.length; dayIndex++){
				...
				if(dayIndex < firstDayWkIndex){
					...
					var isHoliday = checkHoliday(transDate(dt));
					...
					if(isHoliday){
						...
						//FIX BY PONG
						calDayCell.addClass("JFrontierCal-Day-Cell-Week");
						calWeekHeaderCellObj.addClass("JFrontierCal-Week-Header-Cell-Last");
					}
					//fix by pong for sunday cell
					if(isHoliday&& dayIndex < firstDayWkIndex){
						calDayCell.addClass("JFrontierCal-Day-Cell-Other-Week");
						calWeekHeaderCellObj.addClass("JFrontierCal-Week-Header-Cell-Last");
					}else if(isHoliday&& dayIndex >= firstDayWkIndex){
						calDayCell.addClass("JFrontierCal-Day-Cell-Week");
						calWeekHeaderCellObj.addClass("JFrontierCal-Week-Header-Cell-Last");
					}
				}else {
...
//						calDayCell.removeClass();
					...
					if(thisMonth == currentMonthNum){
						if(isHoliday && dayNum != currentDayNum){
							calDayCell.addClass("JFrontierCal-Day-Cell-Week");
							calWeekHeaderCellObj.addClass("JFrontierCal-Week-Header-Cell-Last");
						}
					}else{
...
/*					//fix by pong for sunday cell
					if(isHoliday && dayIndex < firstDayWkIndex && dayNum != currentDayNum){
						calDayCell.addClass("JFrontierCal-Day-Cell-Other-Week");
						calWeekHeaderCellObj.addClass("JFrontierCal-Week-Header-Cell-Last");
					}else if(isHoliday && dayIndex >= firstDayWkIndex && dayNum != currentDayNum){
						calDayCell.addClass("JFrontierCal-Day-Cell-Week");
						calWeekHeaderCellObj.addClass("JFrontierCal-Week-Header-Cell-Last");
					}*/
					dayNum += 1;
			...
			for(var weekIndex = 2; weekIndex < numberWeekRows; weekIndex++){
				...
				for(var dayIndex = 0; dayIndex < Calendar.dayNames.length; dayIndex++){
					...
					var thisMonth = (new Date()).getMonth();
					var isHoliday = checkHoliday(transDate(dt));
					var currentDate = currentYearNum + "-" + (currentMonthNum) + "-" + dayNum;
					calDayCell.setDate(dt);						
					calWeekHeaderCellObj.setDate(dt);
					calWeekHeaderCellObj.setHtml(dayNum+"&nbsp;");
					// add click event handler if the user specified one
...
//						calDayCell.removeClass();
			...
			for(var dayIndex = 0; dayIndex < Calendar.dayNames.length; dayIndex++){
				...
				if(dayNum <= daysInCurrentMonth){
					...
					var currentDate = currentYearNum + "-" + (currentMonthNum+2) + "-" + nextMonthDisplayDayNum;
					calDayCell.setDate(dt);						
					calWeekHeaderCellObj.setDate(dt);
...
/*				if(isHoliday && dayNum <= daysInCurrentMonth&& dayNum != currentDayNum){
					//calDayCell.addClass("JFrontierCal-Day-Cell-Last");
					//FIX BY PONG
					calDayCell.addClass("JFrontierCal-Day-Cell-Week");
					calWeekHeaderCellObj.addClass("JFrontierCal-Week-Header-Cell-Last");
				}else if(isHoliday && dayNum > daysInCurrentMonth&& dayNum != currentDayNum){
...
//					calDayCell.removeClass();
		...
		this.getWeekIndex = function(dayName){
			if(dayName.toUpperCase() == "周日" || dayName.toUpperCase() == "SUNDAY"){
				return 0;
			}else if(dayName.toUpperCase() == "周一" || dayName.toUpperCase() == "MONDAY"){
				return 1;
			}else if(dayName.toUpperCase() == "周二" || dayName.toUpperCase() == "TUESDAY"){
				return 2;
			}else if(dayName.toUpperCase() == "周三" || dayName.toUpperCase() == "WEDNESDAY"){
				return 3;
			}else if(dayName.toUpperCase() == "周四" || dayName.toUpperCase() == "THURSDAY"){
				return 4;
			}else if(dayName.toUpperCase() == "周五" || dayName.toUpperCase() == "FRIDAY"){
				return 5;
			}else if(dayName.toUpperCase() == "周六" || dayName.toUpperCase() == "SATURDAY"){
				return 6;
			}else{
				return -1
			}			
	...
	// static properties
	Calendar.dayNames = new Array("SUNDAY","周一","周二","周三","周四","周五","周六");
	/*
	...
	$.fn.jFrontierCal = function(attr,options) {
        ...
        return this.each(function() {
			...
			// create plugin FIX BY PONG: ADD holiday
	...
	var jFrontierCalPlugin = function(
		...
		/**
		 * set the holidayArray when init fix by pong
		 */
		this.setHolidayArray = function(holiday){
		...
		this.showMonth = function(calId,year,month){
			if(calId != null && year != null && month != null && year.length == 4 && (month.length == 1 || month.length == 2)){
				calId = stripNumberSign(calId);
				// strip any preceeding 0's
				//fix by pong--- while month is January,month = month.replace(/^[0]+/g,"") will replace 0
				if(month != 0){
